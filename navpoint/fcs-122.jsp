...
<section  ><%-- Widget fcs.pages.resources_reportsAndWhitePapers_SpeciesCalendarDownloadSurveySeason@10 (type=tooltwist.fcs.widgets.BannerInnerPageWidget) --%>
<%
try {
	String snippetVar_id = "fcspagesresources_reportsAndWhitePapers_SpeciesCalendarDownloadSurveySeason10BannerInnerPage";
	...
	String snippetVar_widgetPath = "fcs.pages.resources_reportsAndWhitePapers_SpeciesCalendarDownloadSurveySeason@10";
	String snippetVar_elementId = "";
	String snippetVar_idDefinition = "";
%>
		...
		<script id="scriptfcspagesresources_reportsAndWhitePapers_SpeciesCalendarDownloadSurveySeason10BannerInnerPage">/**
 * @author Dino Martin
 */
...
(function() {
	...
	ready(function () {
		try {
			var div = document.getElementById("fcspagesresources_reportsAndWhitePapers_SpeciesCalendarDownloadSurveySeason10BannerInnerPage");
			div.style.backgroundImage = "url(/ttsvr/fcs/images/bannerInner/resource_reports_whitepapers.jpg)";
			div.removeAttribute("id");

			var script = document.getElementById("scriptfcspagesresources_reportsAndWhitePapers_SpeciesCalendarDownloadSurveySeason10BannerInnerPage");
...
WbdSession.addError(jh.getCredentials(), "Rendering widget fcs.pages.resources_reportsAndWhitePapers_SpeciesCalendarDownloadSurveySeason@10 (type=tooltwist.fcs.widgets.BannerInnerPageWidget)", e);
}
%>
</section>
<div  class="row middlePage"><div  class=" col-lg-9 col-md-9 col-sm-7 col-xs-12 articleCont"><div  class="breadcrumbs"><a href="/ttsvr/n/fcs-home">Home</a> &raquo; <a href="/ttsvr/n/fcs-108">Resources</a> &raquo; <a href="/ttsvr/n/fcs-113">Reports and White Papers</a> &raquo; <span>Species Calendar Download for Survey Season</span></div>
<div  class="advanceRichtext"><div class="wt12 wcGray3" ><h1>What do you mean, I missed the Survey Season?&nbsp;</h1>
<p>Have you ever had a project that was postponed because you &ldquo;Missed the Survey Season&rdquo; for a federally- or state-listed threatened and endangered (T&amp;E) species?&nbsp; Do you know when those windows occur?&nbsp;</p>
<p>The survey season is the window of opportunity that a biologist is most likely to find a T&amp;E species within a project site. In the State of California, focused surveys are conducted the same year the California Environmental Quality Act environmental document is submitted.&nbsp; Not only do the survey windows vary in time of year and duration, but the number of surveys required also varies from species to species.&nbsp; Each species has a specific unique activity period and some species are easier to find than others.&nbsp; With so many species, and such a wide range of survey periods and number of surveys, it&rsquo;s nearly impossible to keep it all straight.</p>
<p>FirstCarbon Solutions has put together a simple calendar with survey protocol guidelines for some of the most common focused surveys in Southern California to help you stay ahead of these windows.</p>
<p>Just fill out this form to receive a FREE poster-sized Species Calendar, or download a PDF.</p></div></div>
</div>
<div  class="sec2RightCont col-sh  col-lg-3 col-md-3 col-sm-5 col-xs-12 noPad"><div  class="formsHub"><h3>DOWNLOAD SPECIES CALENDAR</h3>
<form accept-charset="UTF-8" enctype="multipart/form-data" action="https://forms.hubspot.com/uploads/form/v2/147290/920ed62e-b902-4b61-9004-3e4660cce6d5" method="POST" role="form" novalidate>
