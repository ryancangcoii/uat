...
<%@page import="tooltwist.fcs.productionHelpers.SearchProductionHelper"%>

<%
	String jspName = "fcs-379";
	JspHelper jh = JspHelper.getJspHelper(pageContext, jspName);
	WbdProductionHelper productionHelper_fcs_pages_search_11 = null;
try {
	Properties productionHelper_fcs_pages_search_11Params = new Properties();
	productionHelper_fcs_pages_search_11 = new tooltwist.fcs.productionHelpers.SearchProductionHelper(productionHelper_fcs_pages_search_11Params);
	productionHelper_fcs_pages_search_11.callPreFetch(jh);
} catch (Exception e) {
	WbdSession.addError(jh.getCredentials(), "Calling preFetch() for production helper productionHelper_fcs_pages_search_11", e);
}
...
<section  ><%-- Widget fcs.pages.search@10 (type=tooltwist.fcs.widgets.BannerInnerPageWidget) --%>
<%
try {
	String snippetVar_id = "fcspagessearch10BannerInnerPage";
	...
	String snippetVar_widgetPath = "fcs.pages.search@10";
	String snippetVar_elementId = "";
	String snippetVar_idDefinition = "";
%>
		...
		<script id="scriptfcspagessearch10BannerInnerPage">/**
 * @author Dino Martin
 */
...
(function() {
	...
	ready(function () {
		try {
			var div = document.getElementById("fcspagessearch10BannerInnerPage");
			div.style.backgroundImage = "url(/ttsvr/fcs/images/bannerInner/about_us.jpg)";
			div.removeAttribute("id");

			var script = document.getElementById("scriptfcspagessearch10BannerInnerPage");
...
WbdSession.addError(jh.getCredentials(), "Rendering widget fcs.pages.search@10 (type=tooltwist.fcs.widgets.BannerInnerPageWidget)", e);
}
%>
</section>
<div  class="row middlePage"><div  class=" col-lg-9 col-md-9 col-sm-7 col-xs-12 articleCont"><div  class="breadcrumbs"><%-- Widget fcs.pages.search@12 (type=tooltwist.fcs.widgets.RegularPaginatorWidget) --%>
<%
try {
	String snippetVar_id = "fcspagessearch12RegularPaginator";
	String snippetVar_ppn = "5";
	String snippetVar_ipp = "10";
	String snippetVar_widgetName = "@12";
	String snippetVar_widgetPath = "fcs.pages.search@12";
	String snippetVar_elementId = "";
	String snippetVar_idDefinition = "";
%>
...
WbdSession.addError(jh.getCredentials(), "Rendering widget fcs.pages.search@12 (type=tooltwist.fcs.widgets.RegularPaginatorWidget)", e);
}
%>
</div>
<div  id="searchCont" class="advanceRichtext"><%-- Widget fcs.pages.search@11 (type=tooltwist.fcs.widgets.SearchWidget) --%>
<%
try {
	WbdProductionHelper helper = productionHelper_fcs_pages_search_11;
	String snippetVar_test = "dino";
	String snippetVar_widgetName = "@11";
	String snippetVar_widgetPath = "fcs.pages.search@11";
	String snippetVar_elementId = "";
	String snippetVar_idDefinition = "";
%>

		<%@page import="tooltwist.fcs.util.StringUtil"%>
		<%@page import="tooltwist.fcs.productionHelpers.SearchProductionHelper"%>
		<%@page import="tooltwist.misc.JspHelper"%>
		<%@page import="tooltwist.wbd.WbdProductionHelper"%>

		<%
			SearchProductionHelper h = (SearchProductionHelper) helper;
		%>

		<form>
			<input type="hidden" id="searchphraseParam" value="<%= StringUtil.defaultIfBlank(request.getParameter("searchphrase"), "all") %>" />
			<input type="hidden" id="orderingParam" value="<%= StringUtil.defaultIfBlank(request.getParameter("ordering"), "newest") %>" />

		    <fieldset class="word">
		        <label for="searchword">Search Keyword:</label>
		        <input type="text" name="searchword" id="searchword" min="3" max="20" size="30" 
		        	   value="<%= StringUtil.emptyIfBlank(request.getParameter("searchword")) %>" />
		        <input type="submit" value="Search" />
		    </fieldset>
		    <div class="found">
		        <p><strong>Total: <span id="numFound">0</span> results found.</strong>
		        </p>
		    </div>
		...
		<input type="hidden" id="searchresults" value="<%= h.getSearchresults().replaceAll("\"", "&quot;") %>" />
		<div id="search-results" class="paginable-container">
			
		</div>
		<%
} catch (Exception e) {
WbdSession.addError(jh.getCredentials(), "Rendering widget fcs.pages.search@11 (type=tooltwist.fcs.widgets.SearchWidget)", e);
}
%>
</div>
</div>
...
%>
  ...
  <script src="/ttsvr/fcs/js/search.js" type="text/javascript"></script>
