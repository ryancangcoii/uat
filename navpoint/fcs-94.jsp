...
<section  ><%-- Widget fcs.pages.solutions_service_dataProcessing@10 (type=tooltwist.fcs.widgets.BannerInnerPageWidget) --%>
<%
try {
	String snippetVar_id = "fcspagessolutions_service_dataProcessing10BannerInnerPage";
	String snippetVar_banner = "/ttsvr/fcs/images/bannerInner/solution_data_processing.jpg";
	...
	String snippetVar_widgetPath = "fcs.pages.solutions_service_dataProcessing@10";
	String snippetVar_elementId = "";
	String snippetVar_idDefinition = "";
%>
		...
		<script id="scriptfcspagessolutions_service_dataProcessing10BannerInnerPage">/**
 * @author Dino Martin
 */
...
(function() {
	...
	ready(function () {
		try {
			var div = document.getElementById("fcspagessolutions_service_dataProcessing10BannerInnerPage");
			div.style.backgroundImage = "url(/ttsvr/fcs/images/bannerInner/solution_data_processing.jpg)";
			div.removeAttribute("id");

			var script = document.getElementById("scriptfcspagessolutions_service_dataProcessing10BannerInnerPage");
...
WbdSession.addError(jh.getCredentials(), "Rendering widget fcs.pages.solutions_service_dataProcessing@10 (type=tooltwist.fcs.widgets.BannerInnerPageWidget)", e);
}
%>
</section>
<div  class="row middlePage"><div  class=" col-lg-9 col-md-9 col-sm-7 col-xs-12 articleCont"><div  class="breadcrumbs"><a href="/ttsvr/n/fcs-home">Home</a> &raquo; <a href="/ttsvr/n/fcs-24">Solutions</a> &raquo; <a href="/ttsvr/n/fcs-27">By Service</a> &raquo; <span>Data Processing</span></div>
<div  class="advanceRichtext"><div class="wt12 wcGray3" ><h1>Data Processing Services</h1>
<h3>Entrust your documents to a global leader in data processing</h3>
<p>As organizations face increasingly massive amount of information in conducting business, big data management and processing can be invaluable for your sustainability and profitability goals. An effective and accurate data processing and management system streamlines your data-intensive activities, reduces resource use &amp; time loss and drives down operational expenses by eliminating waste and errors.</p>
<p>FirstCarbon Solutions fully integrates consulting and software with world-class expertise in back office data processing so you receive the most comprehensive solutions that meet your unique needs. Specifically, we provide the following services to streamline the processing of your energy, sustainability and environmental data and documents:</p>
<ul>
<li class="first"><a href="/ttsvr/solutions/by-service/data-processing/transaction-data-administration">Transaction Data Administration </a></li>
<li><a href="/ttsvr/solutions/by-service/data-processing/document-management">Document Management</a></li>
<li><a href="/ttsvr/solutions/by-service/data-processing/data-system-conversion">Data System Conversion</a></li>
<li><a href="/ttsvr/solutions/by-service/data-processing/rebate-processing">Rebate Processing</a></li>
<li class="last"><a href="/ttsvr/solutions/by-service/data-processing/utility-bill-processing">Utility Bill Processing</a></li>
</ul>
<p>We collect, collate, and upload your data into either your database or our Sustainability Workbench or <em>ghgTrack</em> tools. This way, your staff gains the time and resources it needs to focus on data analysis and interpretation.</p>
<p>As a part of ADEC Group, a world leader in data processing and management, FirstCarbon Solutions has access to a workforce of more than 5,000 experts and professionals across the globe to ensure your data and documents are safe and well-maintained, performing even complex data processes such as bill validation &amp; verification, large-volume bill sifting &amp; basic error checking to historical data entry for gap analysis and cash recovery data entry for CRC compliance.</p>
<h3>Let our data processing experts do the work for you</h3>
<p>FirstCarbon Solutions undertakes a thorough and detailed procedure for metering and billing accuracy. We manage your utility bills, audit them to make sure you are on the correct rate schedule, and prepare them for prompt payment. We also establish a consolidated bill pay program and produce auditable data management reports for your A/P department within the Payment Card Industry Data Security Standard (PCI DSS) guidelines to secure credit card transactions.</p>
<p>Our experts perform energy assessments and data gap analyses to identify and eliminate process inefficiencies, and we maximize your purchasing power and help you negotiate with energy providers for the best rate schedules to save on costs. Finally, we deliver data authenticity by auditing and validating your energy invoices &ndash; clearly and accurately demonstrating savings that will stand up to detailed external scrutiny.</p>
<p>When it comes to challenges you face in data processing and management, expect nothing less from FirstCarbon Solutions than providing the guidance and support you need to improve your bottom line value in a world increasingly focused on sustainability.</p></div></div>
</div>
