...
try {
	...
	productionHelper_skypepi_pages_portal_p_bstrapTranslationTableAdmin_24 = new tooltwist.skypepi.productionHelpers.LanguageTranslatorProductionHelper(productionHelper_skypepi_pages_portal_p_bstrapTranslationTableAdmin_24Params);
	productionHelper_skypepi_pages_portal_p_bstrapTranslationTableAdmin_24.callPreFetch(jh);
} catch (Exception e) {
	WbdSession.addError(jh.getCredentials(), "Calling preFetch() for production helper productionHelper_skypepi_pages_portal_p_bstrapTranslationTableAdmin_24", e);
}
...
try {
	...
	productionHelper_skypepi_zones_portal_z_header_adminHeader_37Params.setProperty("navpointId", "/ttsvr/n/Translation-Table/skypepi-101");
...
try {
	...
	productionHelper_skypepi_zones_portal_z_header_adminHeader_38Params.setProperty("navpointId", "/ttsvr/n/Translation-Table/skypepi-101");
...
try {
	...
	productionHelper_skypepi_zones_portal_z_header_adminHeader_54Params.setProperty("navpointId", "/ttsvr/n/Translation-Table/skypepi-101");
...
try {
	...
	productionHelper_skypepi_zones_portal_z_header_adminHeader_40Params.setProperty("currentNavpoint", "skypepi-101");
	...
	WbdProductionHelper productionHelper_skypepi_zones_portal_z_responsiveFooterAdmin_72 = null;
try {
	Properties productionHelper_skypepi_zones_portal_z_responsiveFooterAdmin_72Params = new Properties();
	productionHelper_skypepi_zones_portal_z_responsiveFooterAdmin_72Params.setProperty("homeNavPointLink", "/ttsvr/home");
	productionHelper_skypepi_zones_portal_z_responsiveFooterAdmin_72Params.setProperty("menuType", "Footer");
	productionHelper_skypepi_zones_portal_z_responsiveFooterAdmin_72Params.setProperty("currentNavpoint", "skypepi-101");
	productionHelper_skypepi_zones_portal_z_responsiveFooterAdmin_72 = new tooltwist.skypepi.productionHelpers.StudentFunctionsMenuProductionHelper(productionHelper_skypepi_zones_portal_z_responsiveFooterAdmin_72Params);
	productionHelper_skypepi_zones_portal_z_responsiveFooterAdmin_72.callPreFetch(jh);
} catch (Exception e) {
	WbdSession.addError(jh.getCredentials(), "Calling preFetch() for production helper productionHelper_skypepi_zones_portal_z_responsiveFooterAdmin_72", e);
}
...
%>
 ...
 <tr>
  <td  class='magic2' align='left' valign='top'>
...
try {
	...
	String snippetVar_currentNavpoint = "skypepi-101";
...
<% isPublic = false; %>
 ...
 %>				<li class=""><a href="/ttsvr/n/Admin-Home/skypepi-105"><%=h.getAltLang().getString("Admin Home",null,"") %></a></li>
				...
				<li class="active"><a href="/ttsvr/n/Translation-Table/skypepi-101"><%=h.getAltLang().getString("Translation Table",null,"") %></a><span></span></li>
...
<div class='DivWidget page-content standard-width' style=""><%-- Widget skypepi.pages.portal_p_bstrapTranslationTableAdmin@24 (type=tooltwist.skypepi.widgets.LanguageTranslatorWidget) --%>
<%
try {
	WbdProductionHelper helper = productionHelper_skypepi_pages_portal_p_bstrapTranslationTableAdmin_24;
	String snippetVar_widgetName = "@24";
	String snippetVar_widgetPath = "skypepi.pages.portal_p_bstrapTranslationTableAdmin@24";
	String snippetVar_elementId = "";
	String snippetVar_idDefinition = "";
%>
...
<div class="row">
	...
	List<Language> languages = WebUtil.getContextValue(request, CONTEXT_VARIABLE.SITE_LANGUAGES, new ArrayList<Language>());
		for (Language language : languages) {
			...
			String languageName = language.getLanguageTranslation();
			if (languageName.equals("")) {
				languageName = language.getLanguageName();
			}
			
			
			if (h.getLanguage().equals(langStringFileName)) {
				langSelected = "language-selector selected";
				langTitle = language.getLanguageTranslation();
				if (langTitle.equals("")) {
					langTitle = language.getLanguageName();
				}
			}
	%>
		<div class="language-spacing" style="width:<%=languageName.length() + 70 %>px !important;"><a class="language-filter <%=langSelected %>" href="#switchLanguage" value="<%=ProjectUtil.getLanguageStringFileName(request, language.getLanguageCode(), language.getLanguageType())  %>"><%=languageName %></a></div>
	<% } %>
		</td>
	</tr>
	</table> 
...
<table cellpadding="0" cellspacing="0" width="100%" id="lang-table" class="portalTable">
	...
	<tr>
    	...
    	<th width="40%" align="left" valign="top">English</th>
   		<th width="60%" align="left" valign="top"><%=langTitle %></th>
   		<th></th>
    </tr>
    </thead>
...
WbdSession.addError(jh.getCredentials(), "Rendering widget skypepi.pages.portal_p_bstrapTranslationTableAdmin@24 (type=tooltwist.skypepi.widgets.LanguageTranslatorWidget)", e);
}
%>
</div>
<table border='0' cellpadding='0' cellspacing='0'  align='center' valign='top'>
...
<div class='DivWidget float-footer' style=""><table border='0' cellpadding='0' cellspacing='0'  width='100%' align='center' valign='top'>
 <tr>
  <td  class='footerBG' align='center' valign='top'><div class='DivWidget standard-width' style=""><%-- Widget skypepi.zones.portal_z_responsiveFooterAdmin@72 (type=tooltwist.skypepi.widgets.NavBarWidget) --%>
<%
try {
	WbdProductionHelper helper = productionHelper_skypepi_zones_portal_z_responsiveFooterAdmin_72;
	String snippetVar_currentNavpoint = "skypepi-101";
	String snippetVar_menuType = "Footer";
	String snippetVar_homeNavPointLink = "/ttsvr/home";
	String snippetVar_widgetName = "@72";
	String snippetVar_widgetPath = "skypepi.zones.portal_z_responsiveFooterAdmin@72";
	String snippetVar_elementId = "";
	String snippetVar_idDefinition = "";
%>
...
WbdSession.addError(jh.getCredentials(), "Rendering widget skypepi.zones.portal_z_responsiveFooterAdmin@72 (type=tooltwist.skypepi.widgets.NavBarWidget)", e);
}
%>
</div>
</td>
 ...
 <tr>
  <td  class='footerBG' align='center' valign='top'><div class='DivWidget standard-width' style=""></div>
</td>
 </tr>
 <tr>
...
<img onerror="$(this).setStyle({ visibility:'hidden' });" src="/ttsvr/cropImage/skypepi.images.portal_i_positiveLogo.png" border="0">
  ...
  <td  class='magic'></td>
  <td  class='magic'><a href="//www.tooltwist.com"  style=" text-decoration:none; color:#666; margin:0; padding:0;"  target="_blank"><img src='/ttsvr/cropImage/skypepi.images.skypepi_i_tooltwistLogo.png' border='0' /></a></td>
  <td  class='magic'>
<a href="/ttsvr/n/Admin-Home/skypepi-105">
...
<div  class="col-md-1 magic2"><a href="//www.tooltwist.com"  style=" text-decoration:none; color:#666; margin:0; padding:0;"  target="_blank"><img src='/ttsvr/cropImage/skypepi.images.skypepi_i_tooltwistLogo.png' border='0' /></a></div>
</div>
</td>
 </tr>
 <tr>
	...
	return {
		...
		init: function() {
			...
			jQuery(document).on("click", ".icon-edit", function() {
				...
				var val = jQuery(alternate).html();
				jQuery(alternate).html("<input type=\"text\" class=\"text-alternate\" value=\""+val+"\"/>");
			...
			jQuery(document).on("click",".save-transalation", function() {
				console.log("save");
				...
				jQuery.ajax({
					url: "",
					data: {
						op:"skypepi_widgets.languageTranslator.translate",
						stringText:stringVal,
						translate:translateVal
					},
					success: function() {
						parents.find(".save-transalation").html("<label>Saved.</label>");
						setTimeout(function() {
...
jQuery(LanguageTranslator.init()); // Run after page loads</script>
<script type="text/javascript">
