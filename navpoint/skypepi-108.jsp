...
<%@page import="tooltwist.skypepi.productionHelpers.MyStudentListProductionHelper"%>
...
try {
	...
	productionHelper_skypepi_zones_portal_z_header_37Params.setProperty("navpointId", "/ttsvr/n/myStudents/skypepi-108");
...
try {
	...
	productionHelper_skypepi_zones_portal_z_header_38Params.setProperty("navpointId", "/ttsvr/n/myStudents/skypepi-108");
...
try {
	...
	productionHelper_skypepi_zones_portal_z_header_51Params.setProperty("navpointId", "/ttsvr/n/myStudents/skypepi-108");
	...
	WbdProductionHelper productionHelper_skypepi_pages_portal_p_btstrap_myStudentList_16 = null;
try {
	Properties productionHelper_skypepi_pages_portal_p_btstrap_myStudentList_16Params = new Properties();
	productionHelper_skypepi_pages_portal_p_btstrap_myStudentList_16 = new tooltwist.skypepi.productionHelpers.MyStudentListProductionHelper(productionHelper_skypepi_pages_portal_p_btstrap_myStudentList_16Params);
	productionHelper_skypepi_pages_portal_p_btstrap_myStudentList_16.callPreFetch(jh);
} catch (Exception e) {
	WbdSession.addError(jh.getCredentials(), "Calling preFetch() for production helper productionHelper_skypepi_pages_portal_p_btstrap_myStudentList_16", e);
}
...
try {
	...
	productionHelper_skypepi_zones_portal_z_header_54Params.setProperty("currentNavpoint", "skypepi-108");
...
try {
	...
	productionHelper_skypepi_zones_portal_z_responsiveFooter_54Params.setProperty("currentNavpoint", "skypepi-108");
...
.SimpleDiv .someClass {
}/* Styles for widget myStudentList */

.MyStudentList .someClass {
}/* Styles for widget simpleDiv */
...
try {
	...
	String snippetVar_navpointId = "/ttsvr/n/myStudents/skypepi-108";
...
try {
	...
	String snippetVar_currentNavpoint = "skypepi-108";
...
<% boolean isPublic = true;%><% if (h.isParent()) { %> 
				...
				<li class="active"><a href="/ttsvr/n/myStudents/skypepi-108"><%=h.getAltLang().getString("myStudents",null,"") %></a><span></span></li>
<% isPublic = false; %>
 <% } %> 
<% if (h.isParentStudent()) { %> 
				...
				<li class=""><a href="/ttsvr/n/myProgressions/skypepi-70"><%=h.getAltLang().getString("myProgressions",null,"") %></a></li>
...
<div class='DivWidget page-content' style=""><div class='DivWidget standard-width' style=""><%-- Widget skypepi.pages.portal_p_btstrap_myStudentList@16 (type=tooltwist.skypepi.widgets.MyStudentListWidget) --%>
<%
try {
	WbdProductionHelper helper = productionHelper_skypepi_pages_portal_p_btstrap_myStudentList_16;
	String snippetVar_widgetName = "@16";
	String snippetVar_widgetPath = "skypepi.pages.portal_p_btstrap_myStudentList@16";
	String snippetVar_elementId = "";
	String snippetVar_idDefinition = "";
%>
...
<%
	// Get the production helper for this widget
	MyStudentListProductionHelper h = (MyStudentListProductionHelper) helper;
	AltLang lang = h.getAltLang();
	XData data = h.getData(jh);
	XNodes children = h.getChildren();
	Element child = null;
	int ctr = 1;
	String classTag = "";
%>
...
<!-- ********** INSERT HTML HERE ********** -->
<div <%=snippetVar_idDefinition%> class="MyStudentList">
	<div class="bc-container portal-ribbon">
		<div class="ribbon-wrap left-edge fork lblue"><span><%=lang.getString("myStudents", null, "") %></span></div>
		<br />
		<br />
		<br />
		<div class="portalTable">
			<table width="100%">
				...
				<% if (h.getChildren().getNumNodes() > 0) { %>
					<% for (children.first();children.next();) { 
						ctr++;
						classTag = ((ctr % 2) == 0) ? "white" : "blue";
						child = (Element)children.getCurrentNode();
						%>
						<tr class="<%=classTag%>">
							<td><%=child.getAttribute("FirstName")%></td>
							<td><%=child.getAttribute("Surname")%></td>
							<td align="left">
								<a href="javascript:void(0);" onclick="Logout.selectChild('<%=child.getAttribute("ChildID")%>');">
									<img src="/ttsvr/cropImage/skypepi.images.portal_i_logoPerson.png" />
								</a>
							</td>
						</tr>
					<% } %>
...
WbdSession.addError(jh.getCredentials(), "Rendering widget skypepi.pages.portal_p_btstrap_myStudentList@16 (type=tooltwist.skypepi.widgets.MyStudentListWidget)", e);
}
%>
</div>
</div>
